...
const DEFAULTS = {
  ...
  industry: "",
  // System background for report header (recommended <= 200 chars).
  // When empty, AI semantic analysis generates it into findings.ai.json.meta.
  systemBackground: "",
  scan: {
    // '**/*' matches all files; actual files processed are determined by
    // the enabled adapters' extensions.
    // Covers frontend projects and Java backend projects.
    include: ["**/*"],
    exclude: [
      "node_modules/**",
      "miniprogram_npm/**",
    ...
    //           java=Java 后端, yaml=YAML 配置, properties=Properties 配置
...
function normalizeAuditStrictness(value) {
  const fallback = DEFAULTS.rules.auditStrictness
...
function loadConfig(opts = {}) {
  ...
  merged.rules.auditStrictness = normalizeAuditStrictness(merged.rules.auditStrictness)
...
module.exports = { loadConfig, DEFAULTS, normalizeAuditStrictness }
