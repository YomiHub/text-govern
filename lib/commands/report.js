...
const { exitCode, buildStats, sortFindings } = require('../severity');
const logger = require('../logger');
const { RULE_FINDINGS_FILE, AI_FINDINGS_FILE, REPORT_FILE, EXTRACTED_FILE } = require('../constants');
...
async function run(opts = {}) {
  ...
  const outputFile = opts.out
    ...
    : path.join(config.output.dir, REPORT_FILE);
  ...
  const { stats, totalFindings } = generateHtmlReport({
    ruleFindings,
    aiFindings,
    scanMeta,
    config,
  ...
  logger.success(`报告已生成: ${path.relative(cwd, outputFile)}`);
